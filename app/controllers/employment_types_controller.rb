  ...
  def create
    ...
    respond_to do |format|
      if @employment_type.save
        format.html { redirect_to employment_types_url, notice: 'Employment type was successfully created.' }
        format.json { render :index, type: :created, location: employment_types_url }
      else
        format.html { render :new }
        format.json { render json: @employment_type.errors, type: :unprocessable_entity }
      end
    end
  end
  ...
  def update
    respond_to do |format|
      if @employment_type.update(employment_type_params)
        format.html { redirect_to employment_types_url, notice: 'Employment type was successfully updated.' }
        format.json { render :index, type: :ok, location: employment_types_url }
      else
